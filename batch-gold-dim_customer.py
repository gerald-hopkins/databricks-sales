...
# MAGIC %md
# MAGIC # Use a spark stream to basically batch process gold layer so can be used in a Workflow
...
def scd2_upsert_to_gold(batch_df, batch_id):
    ...
    # Check if the Gold table exists using spark.catalog.tableExists
    if spark.catalog.tableExists(gold_table_name):
        ...
        # Ensure first batch is inserted with SCD2 metadata
        gold_table = DeltaTable.forName(spark, gold_table_name)
        gold_table.alias("gold").merge(
            batch_df.alias("silver"),
            "gold.c_custkey = silver.c_custkey AND gold.is_current = True"
...
# Stop the query gracefully after processing
